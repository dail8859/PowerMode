...
INT_PTR CALLBACK abtDlgProc(HWND hwndDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	switch(uMsg) {
		case WM_INITDIALOG:
			...
			Edit_SetText(GetDlgItem(hwndDlg, IDC_VERSION), TEXT("Power Mode v") VERSION_TEXT TEXT(" ") VERSION_STAGE);
			return true;
		case WM_COMMAND:
			switch(LOWORD(wParam)) {
				case IDOK:
					...
					return true;
				case IDC_GITHUB:
					ShellExecute(hwndDlg, TEXT("open"), TEXT("https://github.com/dail8859/PowerMode/"), NULL, NULL, SW_SHOWNORMAL);
					return true;
			}
		case WM_DESTROY:
			DestroyWindow(hwndDlg);
